...
function TimeByNumberDateGet(date)
...
function TimeByNumberShortDateGet(date)
...
-- 获取当天凌晨的时间戳
function ZeroTodayTimestampGet()
...
-- 是否同一年
function IsSameYear(oldTime, newTime)
...
-- 是否同一个月
function IsSameMonth(oldTime, newTime)
    local old = os.date("*t", oldTime) 
    local new = os.date("*t", newTime)
...
-- 是否同一天
function IsSameDay(oldTime, newTime)
    return chessutil.DateDayDistanceByTimeGet(oldTime, newTime) == 0
end
