...
function DailyWelfare:GetDailyWelfareInfo()
    ...
    for i, v in ipairs(self.giftBag) do
        ...
        print("GetDailyWelfareInfo, uid="..self.owner.uid..", id="..v.id..", state="..v.state..", cd="..v.cd
                ..", watchVideoMinusMinute="..v.watchVideoMinusMinute..", rewardInfo="..temp.rewardInfo)
    end
    ...
    for i, v in pairs(rewardInfo) do
        if v > 0 then
            ...
            if rewardType <=2 then
                print("礼包领取前, uid="..self.owner.uid..", money="..self.owner.money..", diamond="..self.owner.diamond..", rewardType="..i..", num="..v)
                UserInfo.AddUserMoney(self.owner, rewardType, v)
                print("礼包领取后, uid="..self.owner.uid..", money="..self.owner.money..", diamond="..self.owner.diamond)
            else
...
function DailyWelfare:GetDailyWelfareReward(welfareId, doubleReward)
    ...
    if self:CalcAndJudgeCanGetReward(welfareId, index) == false then
        return 2, "该福利不是可领取状态", 0, rewardStr, {}
    end
...
function DailyWelfare:CostDiamondToRemoveCd(welfareId)
    ...
    local x, y = math.modf(self.giftBag[currentIndex].cd/60)
