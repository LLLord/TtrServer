module('rechargemgr', package.seeall)
-- 初始化
function Init()
	-- 创建订单数据表，
...
function CreateGameOrder(uid, lobbyId)
	local gameOrder = ""
...
function CmdCreatePlatOrderRequest(laccount, rev)
	...
	local gameOrder = CreateGameOrder(uid, rev.extdata)
...
function CmdUserMaxRechargeGetByUid(uid, timestamp1, timestamp2)
	timestamp1 = timestamp1 or 0
	timestamp2 = timestamp2 or os.time()
	local maxRecharge = 0
	local resList = unilight.chainResponseSequence(unilight.startChain().Table("gameorder").Filter(unilight.a(unilight.eq("uid", uid), unilight.eq("bok", 1), unilight.gt("timestamp", timestamp1), unilight.lt("timestamp", timestamp2))))
